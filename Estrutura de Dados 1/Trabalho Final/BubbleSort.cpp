...
int main(int argc, char *argv[]) {
    ...
    result_file = fopen("bubble_sort.txt", "w");
    if (result_file == NULL) {
    ...
    for (size_index = 0; size_index < 10; size_index++) {
        n = tamanhos[size_index];
        
        char nome_arquivo[99];
        
		sprintf(nome_arquivo, "bubble_sort_%d.txt", n);        
		...
		for (ww=0; ww<1000; ww++) {
			...
			le_arquivo(5000);  /* Lendo o arquivo novamente e armazenando os seus valores em vet */
    ...
    printf("\n\nExecuções concluídas. Os resultados foram armazenados em 'bubble_sort.txt'.\n");
